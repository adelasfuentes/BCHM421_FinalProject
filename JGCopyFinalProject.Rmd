...
mep50_geneIndex <- mep50up_genes %>%
  rbind(mep50down_genes) %>%
  mutate(grouping = "mep50")

geneIndex <- prmt5_geneIndex %>%
  rbind(mep50_geneIndex) %>%
  group_by(grouping)
...
plot_reactive <- reactive({
  ...
  all_go_genes <- bind_rows(
    prmt5up_genes %>% mutate(experiment = "prmt5"),
    prmt5down_genes %>% mutate(experiment = "prmt5"),
    mep50up_genes %>% mutate(experiment = "wdr77"),
    mep50down_genes %>% mutate(experiment = "wdr77")
  )

  target_symbols <- all_go_genes %>% 
