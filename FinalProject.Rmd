...
easyClose = TRUE,
"The user can interact with this data via the search bar and filter drop downs, which allow them to type, or select, any of the displayed variables (e.g, GO_ID, Group, Description, FDR, and Enrichment). With the search bar above the interactive data frame, users can look up results across all columns. Additionally, the user can select how many entries to display on the screen via the drop-down list. This goes up to 100 entries. Lastly, the user can export the data table in two different ways—as a .csv or an Excel file.
...
**Green** coloring under the *Enrichment* category represents low enrichment, while **Orange** represents strong enrichment. **Green** coloring under the *FDR* category represents low significance, while **Orange** represents strong significance."
)
)
}) |>
bindEvent(input$show)
