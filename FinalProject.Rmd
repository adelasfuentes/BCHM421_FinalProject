...
```{r, echo=FALSE}
# used inputPanel() command to set widget to panel
#used selectizeInput() to set the dropdown menu widget so that the user can begin typing a gene name and options will appear as opposed to loading all the genes in a dropdown menu at once
# inputPanel(
#       selectizeInput(
#         inputId  = "selected_genes",                
#         label    = "Select Gene(s) to Plot:",
#         choices  = sort(unique(RNAseq$SYMBOL)),      
#         selected = c("PRMT5", "WDR77"),
#         multiple = TRUE
#       ),
# # used mainPanel() command to set output 
#     mainPanel(
...
# Put plot in its own full-width area:
plotlyOutput("gene_expression_plot", height = "500px")
  ...
  showModal(
    modalDialog(
      ...
      tags$ul(
        ...
        tags$li(tags$b("Color Coding:"), " Upon selecting this gene, the user can see both the change in expression of the PRMT5 gene (represented by the red column) and the change in expression of the WDR77 gene (represented by the orange column) from baseline.")
      )
    )
  )
})
...
```{r, echo=FALSE}
# used renderPlot() so that Shiny recognizes plot
#renderPlot({
# filtered SYMBOL out of RNAseq, but only those symbols within our selected genes group
output$gene_expression_plot <- renderPlotly({
  
  p <- RNAseq %>%                               
    filter(SYMBOL %in% input$selected_genes) %>%
    ggplot(aes(x = SYMBOL, y = logFC)) +
    geom_col(aes(fill = silenced), position = "dodge") +
    labs(
      title = paste(
        "Results for Selected Silenced Genes (N =",
        length(input$selected_genes), ")"
      ),
      y = "logFC",
      x = "SYMBOL"
    ) +
...
plotlyOutput("go_plot", height = "650px")
...
plot_reactive <- reactive({
  ...
  GO_column <- plot_data %>%
    ...
    coord_flip() +
    labs(
      title = paste("Results for Top", input$max_genes, "Genes for", input$go_selector),
      x = "SYMBOL", 
      y = "logFC"
    ) +
...
output$go_plot <- renderPlotly({
  ggplotly(plot_reactive()) %>%
    layout(
      margin = list(l = 100),    # Prevent squishing on left axis
...
renderPlot({
  pheatmap(
    ...
    fontsize_row = 12/length(input$GOID),
    cellheight = 12/length(input$GOID),
    border_color = "gray"
  )
},
res = 150,
height = function() {
  max(1200, nrow(heatmap_data()) * 12)
},
width = 1200
)
